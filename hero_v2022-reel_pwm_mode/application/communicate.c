...
/*STM32拥有两个can控制器，在Init中，can1已被占用，这里使用can2*/
...
{
  return FIRMWARE_VERSION;//返回固件信息，为无符号定值0x06u，so far not clear why
}
/*以下为正式通讯系统函数*/
void communicate_task(void const *argument)
{
  ...
  app = get_sys_cfg();
	/*******************************************************************/
  if (app == CHASSIS_APP)//若接收到当前为底盘调试模式
  {
    protocol_local_init(CHASSIS_ADDRESS);
    protocol_can_interface_register("gimbal_can2", 4096, 1, PROTOCOL_CAN_PORT2, GIMBAL_CAN_ID, CHASSIS_CAN_ID, can2_send_data);//为can2寄存器赋值
    protocol_set_route(GIMBAL_ADDRESS, "gimbal_can2");//设置下一跳接口为云台
  ...
  else
  {
    ...
    protocol_rcv_cmd_register(CMD_RC_DATA_FORWORD, dr16_rx_data_by_can);//接收器寄存器设置
  }
...
static int32_t can2_send_data(uint32_t std_id, uint8_t *p_data, uint32_t len)
{
  return can_msg_bytes_send(&hcan2, p_data, len, std_id);//发送数据函数，8个字节，64位
}
