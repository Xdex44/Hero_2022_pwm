...
static int32_t yaw_gyro_input_convert(struct controller *ctrl, void *input);//陀螺仪航向角输入转换
static int32_t pitch_gyro_input_convert(struct controller *ctrl, void *input);//倾斜角
...
int32_t gimbal_cascade_register(struct gimbal *gimbal, const char *name, enum device_can can)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    gimbal->motor[i].can_id = 0x205 + i; //Set can id for gimbal motors, starts at 0x205 appearantly, makes sense because can_id of chassis start at 0x201, so 0x201,0x202,0x203,0x204, and 0x205 corresponds to CAN of chassis, and 0x205-0x207 corresponds to CAN of gimbal
  }
  ...
  pid_struct_init(&(gimbal->cascade[PITCH_MOTOR_INDEX].inter), 30000, 3000, 35, 0.5, 0);
