...
static int32_t shoot_reel_ctrl(struct shoot *shoot);
...
int32_t shoot_set_reel_speed(struct shoot *shoot, uint16_t reel_spd)
{
  if (shoot == NULL)
    return -RM_INVAL;
  shoot->target.reel_spd[0] = reel_spd;
...
int32_t shoot_execute(struct shoot *shoot)
{
  ...
  // shoot_reel_ctrl(shoot);
  //shoot_block_check(shoot);
  shoot_cmd_ctrl(shoot); // reel ctrl may follow this func
  shoot_reel_ctrl(shoot);

  //pdata = motor_device_get_data(&(shoot->motor));

  // if use pwm, then basically no feedback from reel
  ...
  // if use can, untoggle following line
...
int32_t shoot_disable(struct shoot *shoot)
{
  if (shoot == NULL)
    return -RM_INVAL;
  shoot_set_fric_speed(shoot, 1450, 1450); // both 0 if use single-direction pwm mode
  shoot_set_reel_speed(shoot, 1450);
...
static int32_t shoot_reel_ctrl(struct shoot *shoot)
{
  ...
  VAL_LIMIT(shoot->target.reel_spd[0], REEL_STOP_SPEED, REEL_MAX_SPEED);

  shoot_get_reel_speed(shoot, &(shoot->reel_spd[0]));

  if (shoot->target.reel_spd[0] != shoot->reel_spd[0])
  {
    if (shoot->target.reel_spd[0] < shoot->reel_spd[0])
    {
      shoot->reel_spd[0] -= 1;
  ...
  reel_set_output(shoot->reel_spd[0]);
